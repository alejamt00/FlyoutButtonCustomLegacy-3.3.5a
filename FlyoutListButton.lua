...
function FlyoutListButton:OnReceiveDrag()
	...
	ClearCursor()
	local pickedUp = not self.command or FlyoutButton_SetCursor(self.command, self.value, self.subValue)
...
function FlyoutListButton:OnDragStart()
	...
	if self.command and not FlyoutButton_SetCursor(self.command, self.value, self.subValue) then
		return
	end
